...
let agent = { name: "Portero", system_prompt: "Hola", language: "es-AR", voice: "default", voice_speed: 1, realtime_model: "env-default", enabled: false, updated_at: null as string | null };
let writes: unknown[];
beforeEach(() => {
  writes = [];
  agent = { ...agent, name: "Portero", updated_at: null };
  vi.stubGlobal("fetch", vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (path.endsWith("/agent")) {
      if (init?.method === "PUT") { writes.push(JSON.parse(String(init.body))); agent = { ...JSON.parse(String(init.body)), updated_at: "2026-09-05T12:00:00Z" }; }
...
it("loads and saves agent via the typed API and preserves configuration after remount", async () => {
  ...
  await user.clear(name); await user.type(name, "Recepción");
  await user.click(screen.getByRole("button", { name: "Guardar cambios" }));
  expect(await screen.findByRole("status")).toHaveTextContent(/guardada/i);
  expect(writes).toEqual([{ name: "Recepción", system_prompt: "Hola", language: "es-AR", voice: "default", voice_speed: 1, realtime_model: "env-default", enabled: false }]);
  view.unmount();
